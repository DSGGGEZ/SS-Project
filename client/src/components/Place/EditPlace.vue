<template>
	<div class="container createcafe box">
		...
		<form v-on:submit.prevent="editPlace">
			...
			<p>
				<button type="submit" class="btn btn-success createuser">update Cafe</button>
				<button v-on:click="navigateTo('/places' )" class="btn btn-warning createuser">กลับ</button>
			</p>
		</form>
	</div>
</template>
...
export default {
	...
	methods: {
		async editPlace() {
			try {
				await PlaceService.put(this.place);
				this.$router.push({
					name: "places",
				});
			} catch (err) {
