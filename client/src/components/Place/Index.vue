<template>
	<div class="container createcafe">
		...
		<transition-group name="fade">
			<div v-for="place in places" v-bind:key="place.id" class="blog-list box cafe">
				<!-- <p>id: {{ blog.id }}</p> -->
				<div class="blog-pic">
					<!-- <transition name="fade"> -->
					<div class="thumbnail-pic" v-if="place.thumbnail != 'null'">
						<img :src="BASE_URL+place.thumbnail" alt="thumbnail">
					</div>
					<!-- </transition> -->
				</div>
				...
				<div class="blog-info">
					...
					<p>
						<button class="btn btn-sm btn-info" v-on:click="navigateTo('/place/'+ place.id)">View
							Cafe</button>
						<button class="btn btn-sm btn-warning" v-on:click="navigateTo('/place/edit/'+ place.id)">Edit
							Cafe</button>
						<button class="btn btn-sm btn-danger" v-on:click="deletePlace(place)">Delete</button>
					</p>
				</div>
				<div class="clearfix"></div>
			</div>
		</transition-group>
		<div v-if="places.length === 0 && loading === false" class="empty-blog">
			*** Nodata ***
		</div>
		<div id="blog-list-bottom">
			<div class="blog-load-finished" v-if="places.length === 
...
import Place from '../../../../server/src/models/Place'
let LOAD_NUM = 3
let pageWatcher
export default {
	watch: {
