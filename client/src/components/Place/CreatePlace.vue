<template>
  <div class="container createcafe box">
    ...
    <form v-on:submit.prevent="createPlace">
      ...
      <label>Content</label>
      <p>
        <vue-ckeditor v-model.lazy="place.content" :config="config" @blur="onBlur($event)" @focus="onFocus($event)" />
      </p>
      <label>ColorTone</label>
      <input type="text" v-model="place.theme" />
      <label>Status</label>
...
export default {
  ...
  methods: {
    async createPlace() {
      try {
        this.place.pictures = JSON.stringify(this.pictures);
        await PlaceService.post(this.place);
        this.$router.push({
          name: "places",
        });
      } catch (err) {
